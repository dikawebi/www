...
function reset() { editing.value = null; Object.keys(form).forEach((key) => delete form[key]); resource.value.fields.forEach((field: string) => { form[field] = field === 'is_active' ? true : ''; }); }
function edit(row: any) { editing.value = row; resource.value.fields.forEach((field: string) => { form[field] = row[field] ?? ''; }); }
function submit() { const url = editing.value ? `/app/master/${resource.value.slug}/${editing.value.id}` : `/app/master/${resource.value.slug}`; router[editing.value ? 'put' : 'post'](url, form, { onSuccess: reset }); }
function find() { router.get(`/app/master/${resource.value.slug}`, { q: search.value }, { preserveState: true, replace: true }); }
function remove(row: any) { if (confirm(`Hapus ${row.name ?? 'data ini'}?`)) router.delete(`/app/master/${resource.value.slug}/${row.id}`); }
</script>
...
<template>
    ...
    <AppLayout>
        ...
        <div class="mx-auto max-w-7xl space-y-6">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p class="text-xs font-semibold uppercase tracking-[0.2em] text-amber-600">Master data</p><h2 class="mt-1 text-2xl font-semibold text-stone-950">{{ resource.title }}</h2></div><div class="flex gap-2"><input v-model="search.value" class="rounded-xl border-stone-200 bg-white text-sm" placeholder="Cari..." @keyup.enter="find"><button class="rounded-xl bg-stone-900 px-4 py-2.5 text-sm font-semibold text-white" @click="find">Cari</button><button class="rounded-xl bg-lime-300 px-4 py-2.5 text-sm font-semibold text-stone-900" @click="reset">Tambah</button></div></div>
            <section v-if="editing || Object.keys(form).length" class="rounded-3xl border border-stone-200 bg-white p-6 shadow-sm"><form class="grid gap-4 sm:grid-cols-2" @submit.prevent="submit">
                ...
                <div class="flex gap-2 sm:col-span-2"><button class="rounded-xl bg-amber-400 px-4 py-2.5 text-sm font-semibold text-stone-950">Simpan</button><button type="button" class="rounded-xl border border-stone-200 px-4 py-2.5 text-sm" @click="reset">Batal</button></div>
            </form></section>
            <section class="overflow-hidden rounded-3xl border border-stone-200 bg-white shadow-sm"><div class="overflow-x-auto"><table class="w-full min-w-[720px] text-left text-sm"><thead class="border-b border-stone-200 bg-stone-50 text-xs uppercase tracking-wide text-stone-500"><tr><th v-for="column in columns" :key="column" class="px-5 py-4">{{ label(column) }}</th><th class="px-5 py-4">Aksi</th></tr></thead><tbody class="divide-y divide-stone-100"><tr v-for="row in rows" :key="row.id"><td v-for="column in columns" :key="column" class="px-5 py-4 text-stone-700">{{ row[column] ?? row[column.replace('_id', '_name')] ?? '-' }}</td><td class="space-x-3 px-5 py-4"><button class="text-amber-700" @click="edit(row)">Edit</button><button class="text-red-600" @click="remove(row)">Hapus</button></td></tr><tr v-if="!rows.length"><td :colspan="columns.length + 1" class="p-10 text-center text-stone-500">Belum ada data.</td></tr></tbody></table></div></section>
        </div>
    </AppLayout>
</template>
