...
class InertiaStockController extends Controller
{
    public function stock(): Response
    {
        ...
        return Inertia::render('Stock/Index', ['mode' => 'stock', 'rows' => $rows]);
    }
    ...
    public function transfers(): Response
    {
        ...
        return Inertia::render('Stock/Index', [
            'mode' => 'transfers',
            'rows' => StockTransfer::query()->with(['fromOutlet:id,name', 'toOutlet:id,name', 'items.ingredient:id,name,unit'])->latest()->get(),
    ...
    public function storeTransfer(Request $request): RedirectResponse
    {
        ...
        $transfer = DB::transaction(function () use ($data, $fromOutlet, $user) {
            $transfer = StockTransfer::create([
                ...
                'status' => 'draft', 'created_by' => $user->id, 'transferred_at' => now(), 'note' => $data['note'] ?? null,
            ]);
    ...
    public function opnames(): Response
    {
        abort_unless(RolePermission::can(OutletContext::user(), 'StockOpnameResource', 'view'), 403);
        return Inertia::render('Stock/Index', [
            'mode' => 'opnames',
            'rows' => StockOpname::query()->with(['outlet:id,name', 'items.ingredient:id,name,unit'])->latest()->get(),
            'outlets' => OutletContext::selectableOutletOptions(),
            'ingredients' => Ingredient::query()->where('is_active', true)->orderBy('name')->get(['id', 'name', 'unit']),
        ]);
    }
    ...
    public function storeOpname(Request $request): RedirectResponse
    {
        ...
        $data = $request->validate(['outlet_id' => ['required', 'integer', 'exists:outlets,id'], 'opname_date' => ['required', 'date'], 'note' => ['nullable', 'string'], 'items' => ['required', 'array', 'min:1'], 'items.*.ingredient_id' => ['required', 'integer', 'exists:ingredients,id'], 'items.*.actual_qty' => ['required', 'numeric', 'min:0']]);
